...
export default function Bestseller() {
  ...
  const { bestsellers, loading, error } = useSelector((state) => state.product);

  const queryParams = Object.fromEntries([...searchParams]);

  useEffect(() => {
    dispatch(fetchBestsellers({ ProductType, ...queryParams }));
  ...
  ?   bestsellers.filter(p => p.countInStock > 0).map((product) => (
